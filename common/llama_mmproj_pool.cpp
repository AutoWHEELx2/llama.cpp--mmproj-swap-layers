...
struct llama_mmproj_pool * llama_mmproj_pool_init(
    ...
    for (const auto & m : pool->mappings) {
        ggml_backend_tensor_rebind(m.vision_t, pool->host_buf, m.host_data);
    }
...
bool llama_mmproj_pool_swap_in(struct llama_mmproj_pool * pool, struct llama_context * ctx) {
    ...
    // 3. 采用流水线 (Pipelining) 策略实现 PCIe 全双工并行，彻底防止 VRAM 读写污染
    // 先把 Vision 张量按其占用的被驱逐的 LLM 张量 (evicted_tensors) 进行分组
    ...
    try {
        for (size_t i = 0; i < pool->evicted_tensors.size(); ++i) {
            // 步骤 A：将当前层的 LLM 权重读取回内存 (Device-to-Host)
            // 这个 DMA 拷贝在主线程是阻塞的
            ...
            // 等待前一个块的异步写入 (H2D) 完成，防止线程积压
            if (prev_load_thread.joinable()) {
                prev_load_thread.join();
            }

            // 步骤 B：由于当前层 (第 i 层) 已经安全提取到内存，它的 VRAM 空间已经可以安全覆写
            // 启动后台线程，将对应的 Vision 张量写入该 VRAM (Host-to-Device)
            // 妙点：当下一次循环执行 i+1 层的 D2H 时，刚好与这里的 H2D 在物理上全双工并行！
            prev_load_thread = std::thread([pool, i, &grouped_mappings]() {
                for (const auto & m : grouped_mappings[i]) {
                    ggml_backend_tensor_rebind(m.vision_t, m.gpu_buffer, m.gpu_data);
                    ggml_backend_tensor_set(m.vision_t, m.host_data, 0, m.size); // Push to VRAM
                }
            });
        }

        // 循环结束，确保最后一次的后台写入任务也已完成
        if (prev_load_thread.joinable()) {
            prev_load_thread.join();
        }
    } catch (...) {
        success = false;
        pool->state = llama_pool_state::CORRUPTED;
        LLAMA_LOG_ERROR("%s: exception caught during swap in, setting pool to CORRUPTED state\n", __func__);
    }
...
void llama_mmproj_pool_swap_back(struct llama_mmproj_pool * pool, struct llama_context * ctx) {
    ...
    try {
        ...
        pool->state = llama_pool_state::CORRUPTED;
        LLAMA_LOG_ERROR("%s: exception caught during swap back, setting pool to CORRUPTED state\n", __func__);
    }
...
void llama_mmproj_pool_free(struct llama_mmproj_pool * pool) {
    ...
    for (const auto & m : pool->mappings) {
        if (m.vision_t) {
            ggml_backend_tensor_rebind(m.vision_t, nullptr, nullptr);
        }
    }
